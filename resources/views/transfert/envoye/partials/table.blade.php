...
<tr class="hover-row">
    ...
    <td>
        ...
        @elseif(strtolower($bon->etat) === 'envoyé' || strtolower($bon->etat) === 'en attente' || $bon->etatbontransfertid == 2)
            <span class="status-badge status-pending">{{ $bon->etat ?? 'Envoyé' }}</span>
        @elseif(strtolower($bon->etat) === 'validé' || strtolower($bon->etat) === 'pointé' || $bon->etatbontransfertid == 3)
            <span class="status-badge status-paid">{{ $bon->etat ?? 'Validé' }}</span>
        @else
    ...
    <td style="text-align: center;">
        <div class="action-dropdown" style="position: relative; display: inline-block;">
            <button class="btn btn-outline" style="padding: 4px 8px; height: auto;" onclick="this.nextElementSibling.classList.toggle('show')">
...
@empty
<tr>
    <td colspan="8" style="padding: 60px; text-align: center; color: var(--text-muted);">
        ...
        <div style="font-weight: 600; font-size: 15px; margin-bottom: 8px;">Aucun bon de transfert trouvé</div>
