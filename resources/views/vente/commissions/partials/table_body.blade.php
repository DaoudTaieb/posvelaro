...
<tr class="ticket-row">
    ...
    <td class="amount-cell">{{ number_format($comm->totalnetht ?? 0, 3, ',', ' ') }}</td>
    <td class="amount-cell">{{ number_format($comm->tauxcommission ?? 0, 2, ',', ' ') }} %</td>
    <td class="amount-cell" style="color: var(--primary);">{{ number_format($comm->montant_commission ?? 0, 3, ',', ' ') }}</td>
</tr>
@empty
<tr>
    <td colspan="6" style="text-align: center; padding: 32px; color: var(--text-secondary);">
        <div style="display: flex; flex-direction: column; align-items: center; gap: 12px;">
            <div style="padding: 16px; background: #f8fafc; border-radius: 50%;">
            ...
            <div>
                <p style="margin: 0; font-weight: 600; color: #475569; font-size: 14px;">Aucun ticket trouvé</p>
                <p style="margin: 4px 0 0; font-size: 13px;">Veuillez utiliser les filtres (Date ou Vendeur) pour afficher les commissions.</p>
            </div>
        </div>
    </td>
</tr>
